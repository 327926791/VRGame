...
public class CharacterMovement : MonoBehaviour {
        ...
        if (other.gameObject.name == "exit")
        {
            ...
            if (!restarting) {
                ...
                if (enemies.Length > 0)
                {
                    ...
                    {
                        print("enemies " + enemies[i]);
                        Gun script1 = enemies[i].GetComponent<Gun>();
                        if (script1)
                        {
                            script1.isDead = true;
                            script1.animator.StopPlayback();
                            script1.animator.SetBool("reloadAfterFire", false);

                        }
                        Enemy script2 = enemies[i].GetComponent<Enemy>();
                        if (script1)
                        {
                            script2.isDead = true;
                            script2.animator.SetFloat("walk_forward", -1f);
                            script2.animator.SetFloat("animation_speed", 0f);
                        }
    ...
    public void RestartGame(bool win)
    {
        ...
        coroutine = WaitAndPrint(10);
        StartCoroutine(coroutine);

        
    }

    private IEnumerator WaitAndPrint(int countdown)
    {
        yield return new WaitForSeconds(1);
        countdown--;
        if (countdown > 0)
        {
            CountDown.text = "restart in " + countdown + "s";
            //print(countdown);
            IEnumerator coroutine;
            coroutine = WaitAndPrint(countdown);
            StartCoroutine(coroutine);
        }        
        else if (countdown == 0)
        {
            CountDown.text = "restarting";
